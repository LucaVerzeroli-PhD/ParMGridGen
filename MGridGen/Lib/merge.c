...
void Cycle(CtrlType *ctrl, GraphType *graph, int npasses)
{
   ...
   IMfreeN(3, &graph->pwgts, &graph->pvol, &graph->psurf);
...
void Merge_ARatio(CtrlType *ctrl, GraphType *graph)
{
  ...
  IMfreeN(9, &ptr, &ind, &psurf, &pvol, &pwgts, &elpairs, &ptarget, &degrees, &phtable);
}
...
void Contribute_ARatio(CtrlType *ctrl, GraphType *graph)
{
  ...
  if (nsels == 0) {
    graph->nmoves=-1;
    IMfreeN(6, &ptr, &ind, &psurf, &pvol, &pwgts, &elpairs);
    return;
  }
